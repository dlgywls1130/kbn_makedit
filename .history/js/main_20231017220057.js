...
$('.slide_wrap .slide_list').removeClass('zoomOutEffect');
...
$('.slide_wrap').slick({
    ...
    beforeChange: function(event, slick, currentSlide, nextSlide) {
        $('.slide_wrap .slide_list').eq(currentSlide).removeClass('zoomOutEffect');
        $('.slide_wrap .slide_list').eq(nextSlide).removeClass('zoomOutEffect');
    },
    afterChange: function(event, slick, currentSlide) {
        setTimeout(function() {
            $('.slide_wrap .slide_list').eq(currentSlide).addClass('zoomOutEffect');
        }, 10);
    }
});

// 페이지 로딩시 첫 슬라이드에 zoomOutEffect 클래스 추가
setTimeout(function() {
    $('.slide_wrap .slide_list').eq(0).addClass('zoomOutEffect');
}, 10);

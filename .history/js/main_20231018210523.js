...
let startPercentage = 10;
let progressPercentage = 40;
let progressInterval;

function startProgressBar() {
  clearInterval(progressInterval);
  $('.progress-bar').css('width', startPercentage + '%');
  
  let totalWidth = parseInt($('.progress-container').css('width'));
  let maxProgressWidth = (totalWidth * progressPercentage) / 100;
  let currentWidth = (totalWidth * startPercentage) / 100;
  
  let increment = (maxProgressWidth - currentWidth) / 50;
  
  progressInterval = setInterval(function() {
    currentWidth = parseInt($('.progress-bar').css('width'));
  ...
  }, 40);  // 20을 40으로 바꿔 슬라이드 넘어가는 속도를 느리게 조절
}

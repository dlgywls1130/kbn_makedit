...
$(document).ready(function() {
  // 페이지 로드 시 첫 슬라이드에 줌아웃 효과 적용
  $('.slide_wrap .slide_list').eq(0).addClass('zoomOut');
  ...
  $('.slide_wrap').slick({
      ...
      // 모든 슬라이드의 줌아웃 효과 제거
      $('.slide_wrap .slide_list').removeClass('zoomOut');

      // 새 슬라이드에 줌아웃 효과 적용
      $('.slide_wrap .slide_list').eq(nextSlide).addClass('zoomOut');
  });
});

...
$(document).ready(function() {
  ...
  // 화면 크기에 따른 슬라이드 초기화 함수
  function initializeSlick() {
    if ($(window).width() <= 768) {
      if (!$('.tab-biobank').hasClass('slick-initialized')) {
        $('.tab-biobank').slick({
          slidesToShow: 1,
          slidesToScroll: 1,
          arrows: false,
...
$('.rolling_slide').slick({
  ...
  responsive: [ // 반응형 설정 추가
    {
      breakpoint: 768, // 768px 이하의 화면 크기에서
      settings: {
        slidesToShow: 2, // 슬라이드 2개만 보이도록 설정
        slidesToScroll: 1, // 한 번에 슬라이드 넘기는 갯수
      }
    }
  ]
});

...
$('.slide_wrap').slick({
  ...
  beforeChange: function(event, slick, currentSlide, nextSlide) {
      $('.slide_wrap .slide_list').eq(nextSlide).removeClass('zoomOutEffect'); // 클래스 제거
      setTimeout(function() {
          $('.slide_wrap .slide_list').eq(nextSlide).addClass('zoomOutEffect'); // 클래스 다시 추가하여 애니메이션 재시작
      }, 10); // 짧은 지연을 두어 브라우저가 클래스 변경을 인식하도록 합니다.
  }
});

// 초기 페이지 로딩 시에도 첫 번째 슬라이드에 애니메이션을 적용

...
function startProgressBar() {
    ...
    $('.progress-bar').css('width', startWidth + 'px'); // 바로 시작 위치로 설정

    setTimeout(function() {
        let maxProgressWidth = (totalWidth * progressPercentage) / 100;
        $('.progress-bar').css('width', maxProgressWidth + 'px'); // 목표 너비로 설정
    }, 10);

    setTimeout(function() {
        $('.text-list').slick('slickNext');
        $('.progress-bar').css('width', startWidth + 'px'); // 슬라이드 전환 후 다시 시작 너비로 설정
    }, 3000); // 이 값을 조정하여 프로그레스 바의 전체 소요 시간을 조절할 수 있습니다.
}

...
$(document).ready(function() {
  ...
  // biobank_top button click logic
  ...
  // Initialize slick on page load for mobile views
  initializeSlick();
  ...
  // Sticky biobank_top button inside biobank_tab_wrapper
  var $biobankTop = $('.biobank_top');
    $(window).scroll(function() {
        var wrapperTop = $('.biobank_tab_wrapper').offset().top;
        var wrapperBottom = wrapperTop + $('.biobank_tab_wrapper').height() - $biobankTop.height();

        if ($(window).scrollTop() > wrapperTop && $(window).scrollTop() < wrapperBottom) {
            $biobankTop.fadeIn();
            var newTopPosition = $(window).scrollTop() - wrapperTop + 20;  // +20 for some padding
            $biobankTop.css('top', newTopPosition + 'px');
        } else {
            $biobankTop.fadeOut();
        }
    });

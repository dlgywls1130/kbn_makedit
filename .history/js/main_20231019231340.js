...
$(document).ready(function() {
  ...
  $('.tab-content .tab-biobank').on('afterChange', function(event, slick, currentSlide, nextSlide){
    // currentSlide에는 현재 슬라이드의 인덱스가 있습니다.
    // slick.slideCount에는 전체 슬라이드 수가 있습니다.
    var slideInfo = (currentSlide + 1) + '/' + slick.slideCount;
    // slideInfo를 원하는 위치에 표시하세요. 예를 들면:
    $('#slide-number').text(slideInfo); // '#slide-number'는 슬라이드 번호를 표시할 DOM 요소의 ID입니다.
});

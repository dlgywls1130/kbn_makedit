...
let startPercentage = 0; // 시작 비율을 0으로 설정
let progressPercentage = 30; // 종료 비율을 30으로 설정
...
function startProgressBar() {
  ...
  $('.progress-bar').css('width', startWidth + 'px'); // 시작 위치로 설정

  setTimeout(function() {
    let maxProgressWidth = (totalWidth * progressPercentage) / 100;
    $('.progress-bar').css('width', maxProgressWidth + 'px'); // 최대 너비로 설정
  }, 10); // 약간의 딜레이를 주어 시작 너비에서 최대 너비로 부드럽게 전환될 수 있게 함

  progressInterval = setTimeout(function() {
    $('.text-list').slick('slickNext');
    $('.progress-bar').css('width', startWidth + 'px'); // 슬라이드 전환 후 프로그레스바를 시작 너비로 다시 설정
  }, 3000); // 3초 후에 다음 슬라이드로 전환
}

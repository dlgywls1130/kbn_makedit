...
$(document).ready(function() {
  $('.slide_wrap').slick({
      ...
      init: function() {
          // 슬라이드 초기화 시 첫 번째 슬라이드에 줌아웃 효과 적용
          $('.slide_wrap .slide_list').eq(0).addClass('zoomOutAnimation');
      }
  }).on('beforeChange', function(event, slick, currentSlide, nextSlide) {
      // 모든 슬라이드의 줌아웃 애니메이션 클래스를 제거
      $('.slide_wrap .slide_list').removeClass('zoomOutAnimation');

      // 슬라이드 변경 후 해당 슬라이드에 줌아웃 애니메이션 클래스 추가
      setTimeout(function() {
          $('.slide_wrap .slide_list').eq(nextSlide).addClass('zoomOutAnimation');
      }, 10);
  });
});

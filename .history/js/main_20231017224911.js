...
$('.slide_wrap').on('init', function(event, slick) {
  ...
  $(slick.$slides[0]).css('animation', 'none');
  setTimeout(function() {
      $(slick.$slides[0]).css('animation', 'zoomOut 2s forwards');
  });
}).on('afterChange', function(event, slick, currentSlide) {
  // 애니메이션 초기화
  $(slick.$slides).css('animation', 'none');

  // 현재 슬라이드에만 애니메이션 적용
  setTimeout(function() {
      $(slick.$slides[currentSlide]).css('animation', 'zoomOut 2s forwards');
  });
}).slick({
  slidesToShow: 1,
  arrows: true,

...
function resetProgressBar() {
  $('.progress-bar').css('width', '0px'); // 프로그레스 바를 0으로 초기화
}
...
function startProgressBar() {
  ...
  let increment = (maxProgressWidth - startWidth) / 50;
  ...
  progressInterval = setInterval(function() {
    ...
    if (currentWidth < maxProgressWidth) {
      ...
      resetProgressBar(); // 슬라이드가 변경될 때마다 프로그레스 바를 리셋
    }
  }, 60);
}

$('.text-list').on('beforeChange', function(event, slick, currentSlide, nextSlide){
  resetProgressBar(); // 슬라이드가 변경되기 전에 프로그레스 바를 리셋
});

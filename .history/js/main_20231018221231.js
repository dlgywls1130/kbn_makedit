...
$(document).ready(function() {
  $('.hamburger-menu').click(function() {
      $('.nav-slide').animate({right: "0%"}, 300); 
  });

  // Close the sliding nav when clicking outside of it
  $(document).click(function(event) {
      if (!$(event.target).closest('.nav-slide, .hamburger-menu').length) {
          $('.nav-slide').animate({right: "-100%"}, 300);
      }
  });
});
...
$(document).ready(function() {
  ...
  function updateSlideNumber() {
      var currentSlide = $('.tab-content .tab-biobank').slick('slickCurrentSlide');
      var totalSlides = $('.tab-content .tab-biobank').slick('getSlick').slideCount;
      var slideInfo = (currentSlide + 1) + '/' + totalSlides;
      $('#slide-number').text(slideInfo);
  }

  $('.tab-content .tab-biobank').on('afterChange', function(event, slick, currentSlide, nextSlide){
      updateSlideNumber();
  });
  ...
  function adjustTopButtonPosition() {
      if ($('.tab-content.current').length > 0) {
          var topBoundary = $('.biobank_tab_wrapper').offset().top;
          var bottomBoundary = topBoundary + $('.biobank_tab_wrapper').height() - $('.biobank_top').height();
          
          if ($(window).scrollTop() > topBoundary && $(window).scrollTop() < bottomBoundary) {
              var newPosition = $(window).scrollTop() - topBoundary + 10;
              $('.biobank_top').css('top', newPosition).fadeIn();
          } else {
              $('.biobank_top').fadeOut();
          }
      } else {
  ...
  $('ul.tabs li').click(function() {
      ...
      if ($(this).hasClass('current')) {
          ...
          $('.biobank_top').fadeOut(); // 탭이 닫힐 때 biobank_top 숨기기
      } else {
          ...
          // 스크롤 애니메이션
          $('html, body').animate({
              scrollTop: $('.biobank_tab_wrapper').offset().top
          }, 500);
